...
public class RegisterAndLoginService {
    ...
    public void sendPhoneVerificationCode(String phone){
        ...
        redisUtils.expireKey(phone,5, TimeUnit.MINUTES);
    ...
    public String registerUser(UserPojo userPojo){
        ...
        //1，判断验证码
        if( phone == null || userPojo.getVerificode() == null || !userPojo.getVerificode().equals((valueOperations.get(phone)))){
    ...
    public String login(UserPojo userPojo){
        ...
        if(userList != null && userList.size() > 0){
            ...
            if(userList.get(0).getPassword().equals(DigestUtils.md5DigestAsHex(userPojo.getPassword().getBytes()))){
                ...
                hashOperations.put("alreadyLoginUserList",userList.get(0).getAccount(),userList.get(0));
                redisUtils.persistKey("alreadyLoginUserList");
